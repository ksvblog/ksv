...
if (hasSearchWrapper) {
  ...
  const doSearch = async (searchString) => {
    ...
    let filteredJSON = includeSectionsInSearch.map((section) => {
      const data = jsonData.filter((item) => urlize(item.section) === urlize(section));
      ...
      const handleKeyDown = (event) => {
        if (searchItem.length !== 0) {
          if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
            ...
            if (selectedIndex !== -1) {
              let selectedLink = resItems[selectedIndex].getElementsByClassName('search-title')[0].getAttribute('href');
  ...
  const displayResult = (searchItems, searchString) => {
    const generateSearchResultHTML = (item) => {
      ...
      const highlightResultContent = (content) => {
        ...
        if (matchIndex >= 0) {
          ...
          return matchedContent.replace(regex, (match) => lastWord + `<mark>${match}</mark>`);
        }
    ...
    searchResultInfo && (searchResultInfo.innerHTML = filteredItemsLength > 0 ? `${totalResults} ${totalTime}` : '');
...
const showModal = () => {
  ...
  window.setTimeout(() => document.querySelector('[data-search-input]').focus(), 100);
  if (hasSearchModal) {
    disableBodyScroll();
